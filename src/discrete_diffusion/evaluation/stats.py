...
from evaluation.mmd import compute_mmd, gaussian, gaussian_emd, process_tensor
from scipy.linalg import eigvalsh

PRINT_TIME = True
ORCA_DIR = 'evaluation'  # the relative path to the orca dir
...
def adjs_to_graphs(adjs, features=None):
    graph_list = []
    ...
    for i in range(adjs.shape[0]):
        ...
        if features is not None:
            ...
            for node_id in range(G.number_of_nodes()):
                list_feat_dict[node_id] = feat[node_id].tolist()
            nx.set_node_attributes(G, list_feat_dict, "x")
        G.remove_nodes_from(list(nx.isolates(G)))
        if G.number_of_nodes() < 1 and features is not None:
            print("all 0 matrix")
            G.add_node(1, x=features[0][0].tolist())
        if G.number_of_nodes() < 1 and features is None:
            print("all 0 matrix")
            G.add_node(1)
        # G.remove_edges_from(G.selfloop_edges())
        graph_list.append(G)
