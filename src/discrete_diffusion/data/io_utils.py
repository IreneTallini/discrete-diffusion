...
def load_networkx_data(paths) -> List[nx.Graph]:
    graphs_list = []
    features_list = []
    for path in paths:
        with open(path, "r") as f:
            json_dict = json.load(f)
        graphs_list += nx.adjacency_data(json_dict)

    return graphs_list


def load_TU_dataset(paths: List[Path], dataset_name):
    graphs_list = []
    features_list = []
    G = nx.Graph()
    # load data
...
def load_data(dir_paths, dataset_name, feature_params):
    ...
    graph_list = load_graph_list(dir_paths, dataset_name)
...
def load_graph_list(dir_paths, dataset_name):
    """
    ...
    for dir_path in dir_paths:
        dataset_path = f"{os.path.join(dir_path, dataset_name)}_A.txt"
